...
class EvalBatchSampler(torch.utils.data.Sampler):
    ...
    def __init__(
        ...
        # Prefer eval_ids.json inside the test directory (self-contained); else use data/eval_ids/eval_ids_{name}.json
        in_dir_ids = dataset.directory / "eval_ids.json"
        if in_dir_ids.exists():
            with open(in_dir_ids, "r") as f:
                self.ids = json.load(f)
        elif "worldmem_demo" in str(dataset.directory):
            self.ids = [
                [0, 0, 1024],
                [1, 0, 1024],
                [2, 0, 1024],
        ...
        if isinstance(dataset, DatasetMultiplayer):
            # Each row: [episode_id, bot1_start, bot1_end, bot2_start, bot2_end]
            self.examples = [
                SegmentIdMultiplayer(
